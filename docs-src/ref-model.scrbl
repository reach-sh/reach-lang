...
A @deftech{consensus step} is a graph of @tech{consensus computation} with a unique first computation. A @deftech{consensus computation} either binds @tech{consensus state}, @tech{asserts} a property of the @tech{consensus state}, performs a @tech{transfer}, selects between different next @tech{consensus computations}, or @deftech{commits} to the next @tech{step}.

An @deftech{assert}ion is either: a @deftech{static assertion}, which is an always-true formula; an @deftech{assumption}, which is a true formula if @tech{frontends} behave @tech{honest}ly; a @deftech{requirement}, which is a true formula if @tech{participants} behave @tech{honest}ly; or, a @deftech{possibility assertion}, which is a formula for which there exists some values that @tech{honest} @tech{participants} and @tech{frontends} could submit which results in the truth of the formula. An @deftech{honest} @tech{participant} is one that executes the @tech{steps} specified by the @|DApp|, while an @tech{honest} @tech{frontend} is one that only returns @tech{values} which ensure that all @tech{assumptions} evaluate to the boolean @reachin{true}.

A @deftech{value} is either a @tech{base value} or a statically-sized homogeneous array of @tech{base values}. A @deftech{base value} is either an unsigned integer of 256 bits, a boolean, a string of bytes, or an @tech{account} @tech{address}.
