...
The @seclink["ref-backends-rpc-proto"]{Reach RPC Protocol} is designed to be simple to implement by in languages that support HTTP and JSON interaction.
...
It starts by observing the @litchar{verify} option and informing Python library it uses for HTTPS interaction to turn off warnings.
...
After it makes the call, in the @pyin{while} loop starting on line 51, it inspects the result to determine if it a final answer or an @tech{interactive RPC callback}.
If it is a callback, as indicated by the test on line 55, then it extracts the name of the method, @pyin{p['m']}, and invokes it in the original second argument, @pyin{cbacks}, with the provide arguments.
It replaces the @pyin{p} value with the result of that continuation invocation and continues.

@reachex[#:show-lines? #t py-impl #:link #t
         'only 75 75 "# ..."]

Finally, it returns @pyin{rpc} and @pyin{rpc_callbackss} to the user.
