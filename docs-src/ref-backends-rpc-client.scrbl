...
The entire library is 76 lines of code.
...
On lines 60 and 61, this function inspects its third argument, @pyin{cbacks},
...
invocation.
After it makes the call, in the @pyin{while} loop starting on line 64, it
inspects the result to determine if it is a final answer or an
@tech{interactive RPC callback}.
If it is a callback, as indicated by the test on line 68, then it extracts the
