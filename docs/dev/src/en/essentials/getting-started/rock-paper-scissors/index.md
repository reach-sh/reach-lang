...
While the JavaScript includes an explicit call to the `hasRandom` method in the standard library, in Python, we direct the RPC server to append to the interface with `'stdlib.hasRandom' : True` as a field in the constructor's [return value](https://docs.reach.sh/rps-7-rpc.html#py-return). (Seen in line 42, below).
