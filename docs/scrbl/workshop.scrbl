...
In typical programming contexts, an algorithm that uses @tt{3 log_2 n + 4 n} operations is considered equivalent to an algorithm that uses @tt{5 log_4 n + 22 n} operations, because constants and bases are @link["https://en.wikipedia.org/wiki/Big_O_notation#Properties"]{ignored in asymptoptic analysis}.
...
The exact value of @tt{k} would be chosen empirically based on the relative difference in cost between increased message sizes and computations versus the fixed cost of having any transaction at all on the @tech{consensus network}.
Reach's ability to abstract away the details of communication patterns allows us to write this program abstractly and only specify the value of @tt{k} as a compile-time parameter.
...
In @secref["workshop-trust-fund"], we developed an application where the entire contents of a trust fund devolve to the receipient after a certain time.
...
However, many @tech{consensus networks} also support custom fungible assets that are like tokens.
...
In this workshop, we extend @secref["workshop-nft-dumb"] by incorporating a Harberger Tax (c.f. @link["https://en.wikipedia.org/wiki/Arnold_Harberger"]{Arnold Harberger}) where owners must state a price at which they are willing to part with the asset and pay a percentage of that price to the creator.
