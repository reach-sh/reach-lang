...
@section[#:style 'hidden-number]{0.1.7: 2021/11 - present}

Version 0.1.7 is the current Reach release candidate version.

@itemlist[
@item{2021/12/16: Added @reachin{Events}.}
@item{2021/12/16: The backend interface to the compiled objects was updated, so you'll need to recompile for this release.}
@item{2021/12/16: The backend interface to deployed contracts was updated, so old contracts will not work with this version.}
...
@item{2021/11/19: The backend interface to the compiled objects was updated, so you'll need to recompile for this release.}
@item{2021/11/17: Added @jsin{ctc.unsafeViews} and @jsin{ctc.safeApis}.}
@item{2021/11/15: Algorand connector now uses the application-controlled account for the escrow account.
...
@item{2021/11/15: Algorand-only: The backend interface to deployed contracts was updated, so old contracts will not work with this version.}
]

@section[#:style 'hidden-number]{0.1.6: 2021/10 - 2021/11}
