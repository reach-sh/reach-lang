...
In other words, a Reach program that implemented a poker game could be a game that used @emph{some} token, it would have to be a game that used @emph{Zorkmids} and you'd have to write (or verify) another program to have game that used @emph{Gil}, and so on.
