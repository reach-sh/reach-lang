...
Lines 6 and 7 would not throw an exception if `{!cmd} ./reach run index hello "Mr. Postman"` is run in the terminal.
This is because `{!js} process.argv` would return the following `{!rsh} array`:
