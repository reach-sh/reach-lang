...
const processMd = async ({ baseConfig, relDir, in_folder, iPath, oPath }) => {
	...
	const verCur = (ver) =>
		`Version ${ver} is the current Reach release version.`;
	const verRC = (ver) =>
		`Version ${ver} is the current Reach release candidate version.`;
	const rcHead = (ver) =>
		`_Available in release candidate ${ver} and later..._`;
	...
	const expanderEnv = {
		...
		tooltip,
		directive_note,
		directive_hiddenNote,
		directive_alongside,
		directive_alongsideColumn,
		directive_testQ,
		directive_testA,
		generateIndex,
		verOld,
	...
	const linkifySpans = (elem, language) => {
		...
		const linkifySpan = (s, language) => {
			...
			for (const t of prefixedTexts) {
				if (
					xrefGetIsPrefix(language, t) &&
					!activePrefixes.includes(t)
				) {
	...
	for (const pre of doc.querySelectorAll("pre")) {
			...
			if (arr.length > 0) {
				...
				if (line1.slice(0, 5) == "load:") {
					...
					if (sum_actual !== sum_expected) {
						throw Error(
							`${eprefix}: md5 mismatch: got ${sum_actual}, expected ${sum_expected}`
						);
					}
		...
		if (spec.load) {
			chEl.appendChild(
				mkEl(
					`<a target="_blank" href="${repoBaseNice}${spec.load}">${spec.load}</a>`
				)
			);
		} else {
...
const bookPipe = await unified().use(rehypeStringify);
...
const processAll = async (base_html, inputBaseConfig, folder) => {
	...
	try {
		await fs.unlink(`${out_folder}/index.html`);
	} catch (e) {
		void e;
	}
			...
			if (s.isDirectory()) {
				...
				if (e === "css") {
					...
					return await fs.copyFile(
						path.join(in_folder, p),
						path.join(out_folder, p)
					);
				}
			}
		})
...
const generateSearch = async () => {
	await fs.writeFile(
		`${rootDir}/searchData.json`,
		JSON.stringify(searchData, null, 2)
	);
};

const doTop = () => processAll(`base.html`, process.env, srcDir);
...
await Promise.all([doTop(), generateRedirects()]);
// This depends on the actual content being complete
await Promise.all([generateSearch()]);
