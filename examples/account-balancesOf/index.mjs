...
stdlib.assert((await accAlice.balanceOf(egg.id)).eq(50));
stdlib.assert((await accAlice.balanceOf(bkn.id)).eq(100));
stdlib.assert((await accAlice.balanceOf(tst.id)).eq(150));

const [eggBal, bknBal, tstBal] = await accAlice.balancesOf([egg.id, bkn.id, tst.id]);

stdlib.assert(eggBal.eq(50));
stdlib.assert(bknBal.eq(100));
stdlib.assert(tstBal.eq(150));
