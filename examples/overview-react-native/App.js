...
class App extends React.Component {
  ...
  fundAccount(fundAmountStandard) {
    // from mode: FundAccount
    const {faucet, acc} = this.state;
    const amountAtomic = reach.parseCurrency(
      fundAmountStandard || defaultFundAmtStandard,
    );
  ...
  render() {
    ...
    switch (mode) {
      ...
      case 'FundAccount':
        app = (
          <AppViews.FundAccount
            {...{parent, addr, bal, standardUnit, defaultFundAmtStandard}}
          />
        );
        break;
      case 'SelectRole':
...
class Alice extends React.Component {
  ...
  enterRequest(requestStandard) {
    this.setState({mode: 'RunBackend', requestStandard});
  } // from mode: EnterRequest

  async runBackend() {
    // from mode: RunBackend
  ...
  render() {
    ...
    switch (mode) {
      ...
      case 'EnterRequest':
        alice = (
          <AliceViews.EnterRequest
            {...{parent, standardUnit, defaultRequestStandard}}
          />
        );
        break;
      case 'RunBackend':
        alice = (
          <AliceViews.RunBackend
            {...{parent, info, requestStandard, standardUnit}}
          />
        );
        break;
      case 'BackendRunning':
  ...
  async runBackend(ctcInfoStr) {
    ...
    const interact = {
      want: request =>
        this.setState({
          mode: 'DisplayInfo',
          requestStandard: reach.formatCurrency(request, 4),
        }),
