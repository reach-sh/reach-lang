...
fileUpsert :: FilePath -> BL.ByteString -> App ()
fileUpsert f c = do
  ensureInstall
  liftIO $ BL.writeFile f c
  applyPerms
...
pathLockFile = (</> "lock.json") <$> asks e_dreachp
...
withDotReach m = do
  ...
  let decodeFileThrow f = do
        (liftIO $ eitherDecodeFileStrict f) >>= \case
          Left e -> error $ "Failed to decode JSON: " <> e
          Right v -> return v
