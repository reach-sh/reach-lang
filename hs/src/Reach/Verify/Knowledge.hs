...
data KCtxt = KCtxt { ctxt_mlog :: Maybe Handle
...
know1 ctxt from to = do
  ...
  let f = \case Nothing -> Just $ S.singleton from
...
all_points = \case
  ...
  where more = mconcatMap all_points
...
kgq_m iter ctxt = \case
    ...
    where this =
            case (ct, what) of
              ...
              (CT_Unknowable who, (DLA_Tuple whats)) ->
                ...
                where query_each = query ctxt at f who . all_points
              (CT_Unknowable {}, _) ->
  ...
  LL_LocalIf _ ca t f k ->
    ...
    kgq_l ctxt' f <>
    iter ctxt k
    where ctxt' = ctxt_add_back ctxt ca
...
kgq_s ctxt = \case
  ...
  LLS_ToConsensus _at _from fs from_as from_msg from_amt mtime next_n ->
    ...
    where msg_to_as = mconcatMap (uncurry (kgq_a_only ctxt)) $ zip from_msg from_as
