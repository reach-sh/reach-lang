...
data LLProg
  = LLProg SrcLoc LLOpts SLParts DLInit DLExports DLViews ApiCalls DLAPIs Aliases DLEvents LLStep
  deriving (Eq)

instance HasCounter LLProg where
  getCounter (LLProg _ llo _ _ _ _ _ _ _ _ _) = getCounter llo

instance Pretty LLProg where
  pretty (LLProg _at _ sps dli dex dvs dac dapis alias devts db) =
    "#lang ll" <> hardline
      <> pretty sps
      <> hardline
      <> hardline
