...
data DLProg
  = DLProg SrcLoc DLOpts SLParts DLInit DLSExports DLViews ApiCalls DLAPIs Aliases DLEvents DLStmts
  deriving (Generic)

instance HasCounter DLProg where
  getCounter (DLProg _ dlo _ _ _ _ _ _ _ _ _) = getCounter dlo

instance Pretty DLProg where
  pretty (DLProg _at _ sps dli dex dvs dapic dapis alias devts ds) =
    "#lang dl" <> hardline
      <> pretty sps
      <> hardline
      <> hardline
