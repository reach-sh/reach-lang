...
instance Erase DLStmt where
  el = \case
    ...
    DL_ArrayReduce at ans x z b a i f ->
      isUsedv ans >>= \case
        False -> skip at
        True -> DL_ArrayReduce at ans <$> el x <*> el z <*> pure b <*> pure a <*> pure i <*> el f
    DL_Var at dv ->
      isUsedv dv >>= \case
        False -> skip at
...
instance Erase LLProg where
  el (LLProg at llo ps dli dex dvs das devts s) =
    LLProg at llo ps dli <$> el dex <*> pure dvs <*> pure das <*> pure devts <*> el s
