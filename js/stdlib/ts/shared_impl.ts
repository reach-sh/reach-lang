...
import { CBR_Address, bigNumberify } from "./CBR";
...
export const hasProp = (o: unknown, p: string) =>
  o && {}.hasOwnProperty.call(o, p);
...
export const debug = (...msgs: any) => {
  if (getDEBUG()) {
    ...
    const betterMsgs = msgs.map((msg: any) =>
      ...
      util &&
      util.inspect instanceof Function
...
export type IBackendViews<ConnectorTy extends AnyBackendTy> = {
  ...
  infos: {
    [viewn: string]:
      | TaggedBackendView<ConnectorTy>
      | IBackendViewInfo<ConnectorTy>;
  };
};
...
export type MkPayAmt<Token> = [BigNumber, Array<[BigNumber, Token]>];
...
export type IRecv<RawAddress> =
  | IRecvNoTimeout<RawAddress>
  | {
      didTimeout: true;
    };
...
export type ISendRecvArgs<
  RawAddress,
  Token,
  ConnectorTy extends AnyBackendTy,
  ContractInfo
> = {
  funcNum: number;
  evt_cnt: number;
...
> = {
  ...
  stdlib: Object;
  sendrecv: (
    args: ISendRecvArgs<RawAddress, Token, ConnectorTy, ContractInfo>
...
export type ISetupViewArgs<ContractInfo, VerifyResult> = Omit<
  ISetupArgs<ContractInfo, VerifyResult>,
  "setInfo"
>;

export type ISetupEventArgs<ContractInfo, VerifyResult> = Omit<
  ISetupArgs<ContractInfo, VerifyResult>,
  "setInfo"
>;
...
export type ISetupRes<
  ContractInfo,
  RawAddress,
  Token,
  ConnectorTy extends AnyBackendTy
> = Pick<
  IContractCompiled<ContractInfo, RawAddress, Token, ConnectorTy>,
  SpecificKeys
>;

export type IStdContractArgs<
  ContractInfo,
  VerifyResult,
  RawAddress,
  Token,
...
> = {
  ...
  _setup: (
    args: ISetupArgs<ContractInfo, VerifyResult>
  ) => ISetupRes<ContractInfo, RawAddress, Token, ConnectorTy>;
} & Omit<
  IContractCompiled<ContractInfo, RawAddress, Token, ConnectorTy>,
  SpecificKeys
>;
...
export type ISetupView<
  ContractInfo,
  VerifyResult,
  ConnectorTy extends AnyBackendTy
> = (args: ISetupViewArgs<ContractInfo, VerifyResult>) => {
  viewLib: IViewLib;
  getView1: (
    views: IBackendViewsInfo<ConnectorTy>,
    v: string,
    k: string | undefined,
    vi: IBackendViewInfo<ConnectorTy>,
    isSafe: boolean
  ) => ViewVal;
};
...
export const stdVerifyContract = async <ContractInfo, VerifyResult>(
  stdArgs: Pick<
    ISetupViewArgs<ContractInfo, VerifyResult>,
    "getTrustedVerifyResult" | "setTrustedVerifyResult"
  >,
...
export const stdContract = <
  ...
  ConnectorTy extends AnyBackendTy
>(
  stdContractArgs: IStdContractArgs<
    ContractInfo,
    VerifyResult,
    RawAddress,
    Token,
  ...
  const { setInfo, getInfo }: SomeSetupArgs = (() => {
    let _setInfo = (info: ContractInfo) => {
      throw Error(
        `Cannot set info(${j2s(
          info
        )}) (i.e. deploy) when acc.contract called with contract info: You are trying to attach to a contract as the deployer, which is not possible.`
      );
      return;
    };
  ...
  const setTrustedVerifyResult = (x: any) => {
  ...
  const mkViews = (isSafe: boolean) =>
    objectMap(
      ...
      (
        v: string,
        vm: TaggedBackendView<ConnectorTy> | IBackendViewInfo<ConnectorTy>
      ) =>
        isUntaggedView(vm)
          ? getView1(
              views_bin.views,
              v,
              undefined,
              vm as IBackendViewInfo<ConnectorTy>,
              isSafe
            )
          : objectMap(
              vm as TaggedBackendView<ConnectorTy>,
              (k: string, vi: IBackendViewInfo<ConnectorTy>) =>
  ...
  const events = objectMap(eventMap, (k: string, v: any) =>
    Array.isArray(v) // untagged
      ? createEventStream(k, v)
      : objectMap(v, (kp, vp: any) => createEventStream(k + "_" + kp, vp))
  );
  ...
  return {
    ...
    getViews: () => {
      console.log(
        `WARNING: ctc.getViews() is deprecated; use ctc.views or ctc.v instead.`
      );
      return views;
    },
    unsafeViews,
...
>(
  ...
  IAccount<NetworkAccount, Backend, Contract, ContractInfo, Token>,
  "setGasLimit" | "getGasLimit" | "setStorageLimit" | "getStorageLimit"
> => {
...
>(
  ...
  return {
    ...
    deploy: (bin: Backend) => {
      console.log(
        `WARNING: acc.deploy(bin) is deprecated; use acc.contract(bin) instead. Deployment is implied by the first publication.`
      );
      return orig.contract(bin, undefined);
    },
    attach: (bin: Backend, ctcInfoP: Promise<ContractInfo>) => {
      console.log(
        `WARNING: acc.attach(bin, info) is deprecated; use acc.contract(bin, info) instead. Attachment is implied by reception of the first publication.`
      );
...
export type IAccountTransferable<NetworkAccount> =
  | IAccount<NetworkAccount, any, any, any, any>
  | {
      networkAccount: NetworkAccount;
    };
 ...
 */
export function replaceableThunk<T>(
  thunk: () => T
): [() => T, (val: T) => void] {
  let called = false;
  let res: T | null = null;
...
export function truthyEnv(v: string | undefined | null): v is string {
  if (!v) return false;
  return !["0", "false", "f", "#f", "no", "off", "n", ""].includes(
...
export const envDefaultNoEmpty = <T>(
...
export const hexToBigNumber = (h: string): BigNumber =>
  bigNumberify(hexTo0x(h));
...
export const makeArith = (m: BigNumber): Arith => {
  ...
  const checkM = (x: BigNumber) => checkedBigNumberify(`internal`, m, x);
  ...
  const liftX2 =
    (check: (x: BigNumber) => BigNumber) =>
    (f: BNOp2) =>
    (a: num, b: num): BigNumber =>
      check(doBN2(f, bigNumberify(a), bigNumberify(b)));
...
export const argsSplit = <T>(
  args: Array<T>,
  cnt: number
): [Array<T>, Array<T>] =>
  cnt == 0
    ? [args, []]
    : [args.slice(0, args.length - cnt), args.slice(-1 * cnt)];

export const objectMap = <A, B>(
  object: { [key: string]: A },
  mapFn: (k: string, a: A) => B
): { [key: string]: B } =>
  Object.keys(object).reduce(function (
    result: { [key: string]: B },
    key: string
  ) {
...
export const mkAddressEq =
  (T_Address: { canonicalize: (addr: any) => any }) =>
  (x: any, y: any): boolean =>
    bytesEq(T_Address.canonicalize(x), T_Address.canonicalize(y));

export const ensureConnectorAvailable = (
  bin: any,
  conn: string,
  jsVer: number,
  connVer: number
  ...
  if (!conn_bin) {
    throw new Error(
      `The application was not compiled for the ${conn} connector, only: ${Object.keys(
        connectors
      )}`
    );
  }
...
export const checkVersion = (
  ...
  if (actual !== expected) {
    ...
    const more = older
      ? `update your compiler and recompile!`
      : `update your standard library and rerun!`;
    throw Error(
      `This Reach compiled ${label} does not match the expectations of this Reach standard library: expected ${expected}, but got ${actual}; ${more}`
    );
  }
};
...
const argHelper = (
  ...
  return xs.reduce(
    (accum: any, x: any) => (op(f(x), f(accum)) ? x : accum),
    xs[0]
  );
};
...
export const make_newTestAccounts = <X>(
  newTestAccount: (bal: any) => Promise<X>
): {
...
export const make_waitUntilX =
  (
    label: string,
    getCurrent: () => Promise<BigNumber>,
    step: (target: BigNumber) => Promise<BigNumber>
  ) =>
  async (target: BigNumber, onProgress?: OnProgress): Promise<BigNumber> => {
...
export const checkTimeout = async (
  runningIsolated: () => boolean,
  getTimeSecs: (now: BigNumber) => Promise<BigNumber>,
  timeoutAt: TimeArg | undefined,
  nowTime: BigNumber
): Promise<boolean> => {
...
export interface IEventQueue<EQInitArgs, RawTxn, ProcTxn> {
  ...
  peq: (
    lab: string,
    didTimeout: AsyncPred<Time>
  ) => Promise<EQPeqResult<ProcTxn>>;
...
export interface EQCtorArgs<EQInitArgs, RawTxn, ProcTxn> {
  ...
  getTxns: (
    ...
    ctime: Time,
    howMany: number
  ) => Promise<EQGetTxnsR<RawTxn>>;
  getTxnTime: (x: RawTxn) => Time;
}
export const makeEventQueue = <EQInitArgs, RawTxn, ProcTxn>(
  ctorArgs: EQCtorArgs<EQInitArgs, RawTxn, ProcTxn>
): IEventQueue<EQInitArgs, RawTxn, ProcTxn> => {
  ...
  const peq = async (
    ...
    while (ptxns.length === 0) {
      ...
      while (txns.length > 0 && cis.length > 0) {
        ...
        if (!ci(t)) {
          throw Error(
            `${dhead} customIgnore present, ${ci}, but top txn did not match ${j2s(
              t
            )}`
          );
        } else {
...
export const makeEventStream = <EQInitArgs, RawTxn, ProcTxn, Log>(
  args: IMESArgs<EQInitArgs, RawTxn, ProcTxn, Log>
) => {
    ...
    while (parsedLog === undefined) {
      while (logs.length === 0) {
        const timeout = maxTime
          ? async (t: BigNumber) => t.gt(maxTime)
          : neverTrue;
  ...
  const monitorUpToTime = async (maxTime: Time, onEvent: (x: any) => void) => {
    while (true) {
      const r = await nextUpToTime(maxTime);
      if (r === undefined) {
        return;
      }
      onEvent(r);
...
export type NotifySend<A, B> = (
  e: any,
  pre: Promise<A>
) => Promise<[A, NotifyComplete<B>]>;

export const makeSigningMonitor = <A, B>(): [
  SetSigningMonitor,
  NotifySend<A, B>
] => {
...
export const handleFormat = (
  ...
  if (!(Number.isInteger(splitValue) && 0 <= splitValue)) {
    throw Error(
      `Expected split value to be a nonnegative integer, but got ${decimals}.`
    );
  }
...
export const apiStateMismatchError = (
  bin: IBackend<any>,
  es: BigNumber,
  as: BigNumber
): Error => {
  ...
  return Error(
    `Expected the DApp to be in state ${es}, but it was actually in state ${as}.\n` +
