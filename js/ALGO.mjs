...
//   recv: function
// }
const log_pretty = (obj) => console.log(JSON.parse(JSON.stringify(obj)));
...
export const transfer = async (from, to, value) => {
  ...
  const txn = algosdk.makePaymentTxnWithSuggestedParams(sender.addr, receiver, value, undefined, note, params);
...
export function parseCurrency(cm) {
  if (cm.ALGO === undefined) {
    throw Error(`Expected ALGO in ${Object.keys(cm)}`);
  }
  const amt = isBigNumber(cm.ALGO) ? cm.ALGO.toNumber() :
